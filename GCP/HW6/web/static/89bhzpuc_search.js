/*jshint esversion: 6 */
let tomorrowWeatherStore = {}; //global to store and retrieve weather info
...
function call_tomorrow_weather(lat, lng, loc) {
    ...
    get_api_info("tomorrow", "timelines", req_data, function(rspObj){
        ...
        // weather for today
        var currentTimeIdx = getTimestepIdxByName(tomorrowWeatherStore.data.timelines, "current");
        var startTimeIdx = 0;
...
function showForecastDetail(elem){
    var startTimeIdx = jQuery(elem.currentTarget).attr("data-timeidx");
...
let iconPrefix = "web/images/";
let weatherCodeLookup = {
    "0": ["N/A", ""],
...
function get_api_info(api, method, req_data, callback){
    ...
    var url = "/apis/" + api + "?" + jQuery.param(req_data);
...
function get_current_weather_icon(infoType){
    var lookup = {
        ...
        "windSpeed":        iconPrefix + "Wind_Speed.png",
